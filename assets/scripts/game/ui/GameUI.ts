...
export default class GameUI extends BaseUI {
    ...
    onToggleHide(){
        console.log("GameUI:onToggleHide")
    }

    onToggleShow(){
        console.log("GameUI:onToggleShow")
    ...
    addEgg(){
        ...
        this.center.addChild(egg)
        this.curEgg = egg
        let posX = this.chicken.position.x + 50
        let posY = this.chicken.position.y - 50
    ...
    showLine(){
        ...
        let localPos = this.center.convertToNodeSpaceAR(cc.v2(midX, midY));

        let crackLine = cc.instantiate(this.crackLine)
        this.center.addChild(crackLine)
    ...
    launchEggFromCrack() {
        ...
        cc.log("鸡蛋从裂缝出来scale",targetScale);
        curEgg.scale = 1/targetScale;
